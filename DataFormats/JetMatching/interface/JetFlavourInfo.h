 ...
 */
    ...
    void setAlgoFlav(const FlavAlgo& algo,
      ...
      if (flavVec.size() != kAlgoArrayLen)
        throw cms::Exception("JetFlavourInfo") << "Expected seven flavour components";
    ...
    int getAlgoFlavLeading(const FlavAlgo& algo) const {
        ...
        for (size_t i = kAlgoArrayLen - 1; i > 0; --i) {
          if (arr[i] != 0)
            return arr[i] > 0 ? static_cast<int>(i) : -static_cast<int>(i);
        }
    ...
    uint32_t getAlgoFlavCode(const FlavAlgo& algo) const {
      ...
      for (unsigned int i = 1; i < kAlgoArrayLen; ++i) {
        ...
        uint32_t flavAbs =
            static_cast<uint32_t>(std::abs(static_cast<int64_t>(arr[i])));
