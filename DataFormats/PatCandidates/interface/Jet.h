...
namespace pat {
  ...
  namespace io_v1 {
    class Jet : public PATObject<reco::Jet> {
      ...
      friend class pat::PATJetProducer;
      friend class pat::PATJetSlimmer;
      friend class pat::PATJetUpdater;
    ...
    public:
      ...
      double groomedMass(unsigned int index = 0) const {
        ...
        return nSubjetCollections() > index && !sub.empty()
                   ? std::accumulate(sub.begin(),
                                     sub.end(),
                                     reco::Candidate::LorentzVector(),
                                     [](reco::Candidate::LorentzVector const& a, reco::CandidatePtr const& b) {
                                       return a + b->p4();
                                     })
                         .mass()
                   : -1.0;
...
inline float pat::io_v1::Jet::chargedHadronEnergy() const {
  if (isPFJet()) {
...
inline float pat::io_v1::Jet::neutralHadronEnergy() const {
  if (isPFJet()) {
...
inline float pat::io_v1::Jet::chargedEmEnergy() const {
  if (isPFJet()) {
...
inline float pat::io_v1::Jet::neutralEmEnergy() const {
  if (isPFJet()) {
...
inline int pat::io_v1::Jet::muonMultiplicity() const {
  if (isPFJet()) {
...
inline int pat::io_v1::Jet::chargedMultiplicity() const {
  if (isPFJet()) {
