...
public:
  ...
  void drawDeck(double x, UniformRng flat, GaussRng gauss, int& K, std::vector<double>& theta) const;

  // Expected energy density of the drawn deck at (cell x, y, layer),
  // in the incidence frame. Caller applies quanta + transplant.
  double density(const std::vector<double>& theta, int K, int layer, double cx, double cy) const;
