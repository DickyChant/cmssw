...
private:
  ...
  edm::EDGetTokenT<reco::GenParticleRefVector> finalPartonsToken_;  // Only consumed when IFN is enabled
...
void JetFlavourClustering::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (ifnParams_.enabled)
    insertFlavouredGhosts(finalPartons, ghostRescaling_, true, std::back_inserter(ghostFinalPartons));
  if (useLeptons_) {
    ...
    if (reclusteredIndices.at(i) < 0) {
      ...
      if (ifnParams_.enabled) {
        if (ifnIndices.at(i) < 0) {
          (*jetFlavourInfos)[jets->refAt(i)].clearAlgoFlav(reco::FlavAlgo::kIFN);
        } else {
